...
/* emp 테이블에서 사원 이름과 급여와 입사일만을 출력하는 쿼리문을 작성하시오. */
select ename, sal, hiredate from emp;

-- 별침 : 컬럼을 기술한 바로 뒤에 as 라고 쓴 후 별칭(별명, 닉네임)을 기술
-- 별칭에는 공백문자, 특수문자($, _, #)등 문자를 표현하고 싶거나 대소문자를 구별하고 싶을때 ""을 사용해여 기술한다.
...
문] 입사일이 85년 이후인사람의 정보(이름, 입사일)을 출력하시오.
*/
...
논리 연산자
 ...
 - 두가지 조건이 모두 만족을 해야함
 OR
 - 두가지 조건중 한가지만 만족하더라고 검색할 수 있음
 NOT
 - 조건에 만족하지 못하는 것만 검색함
 BETWEEN AND
 - 특정 범위 내에 데이터를 검색하고자 할때 사용함
 IN
 - 동일한 컬럼이 여려개 값중 하나인지 알아보고자 할때 사용함
*/
-- 부서번호가 10번이고, 직급이 MANAGER인 사원을 정보를 출력하시오.
select * from emp where deptno=10 and job = 'MANAGER';
-- 급여가 1000에서 3000 사이인 사원들의 정보를 출력하시오.
select * from emp where sal >= 1000 and sal <= 3000;

-- 부서번호가 10번이거나 직급이 MANAGER인 사원을 정보를 출력하시오.
...
/*
Like 연산자
    - 검색 하고자 하는 값을 정확히 모를 경우 와일드 카드와 함계 사용하여
      원하는 내용을 검색하는 연산자
...
-- 사원의 이름이 K로 시작하는 사원을 겁색하여 정보를 출력하시오.
select ename from emp where ename like 'K%';
-- %는 몇개의 문자가 오든 상관이 없다.
select ename from emp where ename like '%K%';

-- 이름중에 두번째 글자 A인 사원의 정보를 출력
...
-- null 이 저장되어 있는 경우에는 = 연산자를 사용할 수 없다.
...
NULL을 위한 연산자
  ...
  - 특정 컬럼 값인지는 비교할 경우에는 비교연산자(=)를 사용하지 않고 is null 연산자를 사용함
  - null 값이 아닌지를 알아보려고 할때는 비교연산자(<>)를 사용하지 않고 is not null 연산자를 사용함
...
/*
정렬을 위한 절은 로우(행)를 정렬하는데 사용함
  order by 절은 로우(행)을 정렬하는데 사용함
  쿼리문 맨뒤에 기술하며 정렬의 기준이 되는 컬럼 이름 또는 select절에서 명시된 별칭을 사용함
...
모든 명령어의 끝은 세미콜론으로 끝남             모든 명령어는 엔터키에 의해 끝남(; 으로 가능함)
                                            명령라인이 계속이어질 경우 라인의 끝에-(하이픈)을 추가함
약어를 사용 가능                              약어 사용 가능
*/
select * from emp where deptno = 10;
/*
